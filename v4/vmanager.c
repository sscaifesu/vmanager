...
char* fetch_api_response(Config *config, const char *endpoint);
cJSON* fetch_api_json(Config *config, const char *endpoint);
...
char* fetch_api_response(Config *config, const char *endpoint) {
    ...
    while (fgets(line, sizeof(line), fp) != NULL && total < 65535) {
        size_t len = strlen(line);
        if (total + len < 65535) {
            strcat(response, line);
            total += len;
...
// 获取 API 响应并解析为 cJSON
cJSON* fetch_api_json(Config *config, const char *endpoint) {
    char *response = fetch_api_response(config, endpoint);
    if (response == NULL) {
        return NULL;
    }
